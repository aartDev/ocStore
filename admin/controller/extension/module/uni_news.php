...
class ControllerExtensionModuleUniNews extends Controller {
	...
	private function getCategoryList() {
		...
		$data['sort_name'] = $this->url->link('extension/module/uni_news/category_list', 'user_token=' . $this->session->data['user_token'] . '&sort=cd.name' . $url, true);
		$data['sort_sort_order'] = $this->url->link('extension/module/uni_news/category_list', 'user_token=' . $this->session->data['user_token'] . '&sort=c.sort_order' . $url, true);
		$data['sort_status'] = $this->url->link('extension/module/uni_news/category_list', 'user_token=' . $this->session->data['user_token'] . '&sort=c.status' . $url, true);
	...
	private function getCategoryForm() {
		...
		$data['error_warning'] = isset($this->error['warning']) ? $this->error['warning'] : '';
		$data['error_warning'] = isset($this->error['name']) ? $this->error['name'] : '';
		$data['error_description'] = isset($this->error['description']) ? $this->error['description'] : '';
	...
	private function validateCategoryForm() {
		...
		foreach ($this->request->post['description'] as $language_id => $value) {
			if ((strlen($value['name']) < 3) || (strlen($value['name']) > 250)) {
				$this->error['name'] = $this->language->get('error_title_category');
			}
		}
