...
class ControllerInformationUniNewsStory extends Controller {
	...
	public function index() {
		...
		if ($news_info) {
			...
			if($news_info['image']) {
				$data['thumb'] = isset($settings['image']) ? $this->model_tool_image->resize($news_info['image'], $settings['thumb_width'], $settings['thumb_height']) : '';
			...
			$data['microdata'] = [
				...
				'description' 		=> strip_tags($news_info['description']),
				...
				'publisher_logo'	=> (is_file(DIR_IMAGE . $this->config->get('config_logo'))) ?  $server.'image/'.$this->config->get('config_logo') : ''
			];
			
			//related products
			$data['type_view'] = isset($settings['related_product_type_view']) ? 'grid' : 'carousel';
			$data['related_products_title'] = isset($settings[$lang_id]['related_product_title']) ? $settings[$lang_id]['related_product_title'] : $this->language->get('text_related_product_title');
