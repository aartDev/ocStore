...
class ControllerExtensionModuleUniQuickOrder extends Controller {
	public function index() {
		if(isset($this->request->post['flag'])) {
			...
			if($product_info) {
				...
				$data['show_attr'] = isset($uniset['quick_order_attr']) ? $uniset['quick_order_attr'] : '';
				$data['description_on'] = isset($uniset['quick_order_description']) ? $uniset['quick_order_description'] : '';
				...
				$data['description'] = utf8_substr(strip_tags(html_entity_decode($product_info['description'], ENT_QUOTES, 'UTF-8')), 0, $uniset['quick_order_description_item']);
				...
				$data['phone_mask'] = isset($uniset['quick_order_phone_mask']) ? $uniset['quick_order_phone_mask'] : '';
		...
		if ($product_info) {
			...
			if($this->config->get('config_checkout_id') && isset($uniset['quick_order_confirm'])) {
				...
				if ($information_info && !isset($this->request->post['confirm'])) {
					$json['error_c']['confirm'] = sprintf($this->language->get('error_agree'), $information_info['title']);
				}
			}
			...
			if (!$json) {
				...
				$json['success']['status'] = $this->add_order($user_data);
				$json['success']['products'] = $products;
				$json['success']['order_id'] = $this->session->data['order_id'];
				$json['success']['text'] = $this->language->get('text_quick_order_success');
			}
		}
	...
	public function add_order($user_data) {
