...
class ModelExtensionModuleUniNews extends Model {
	...
	public function getNews($data) {
		...
		$sql .= " LEFT JOIN `".DB_PREFIX."uni_news_story_to_store` n2s ON (n.news_id = n2s.news_id) WHERE nd.language_id = '".$language_id."' AND n2s.store_id = '".$store_id."' AND n.status = '1'";	
	...
	public function getPrevNextNews($news_id, $date_added, $category_id) {
		...
		if($category_id) {
			...
			foreach($types as $key => $type) {
				...
				$sql .= " WHERE n2c.category_id = '".(int)$category_id."' AND n.date_added ".$this->db->escape($type['sign'])." '".$this->db->escape($date_added)."' AND n.status = '1' AND n2s.store_id = '".(int)$this->config->get('config_store_id')."'";
