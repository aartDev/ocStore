...
class ModelExtensionModuleUniNewData extends Controller {
	...
	public function getNewData($result = [], $setting = []) {
		...
		$data['special_date_end'] = $this->getSpecialDateEnd($result['product_id'], $result['special']);
		...
		if(!isset($result['product_page'])) {
			...
			$quantity = $result['options_quantity'];
	...
	private function getAdditionalImage($product_id, $img_width, $img_height) {
		...
		if(isset($uniset['show_additional_image'])) {
			...
			if(isset($results[0]['image'])) {
				$image = $this->model_tool_image->resize($results[0]['image'], $img_width, $img_height);
			}
		}
	...
	private function getSpecialDateEnd($product_id, $special) {
		$uniset = $this->uniset;
		
		$date_end = '';
		
		if(isset($uniset['show_special_timer']) && $special) {
